...
//!
//! [`OrderComplex::place`] is generic only over `P: OrderRepoProxy`, the proxy
//! trait generated by `#[drive(...)]` from the repo's `run(...)`/`step(...)`
//! oper lists. It has no dependency on `Run`, `Step`, a repository, or a
//! transaction context. Each usecase selects an execution model by
//! constructing a concrete `P`:
//!
//! ```text
//! customer repo  ─┐
...
/// The `OrderRepoProxy` bound is generated by `#[drive]` from the same oper
/// list as `OrderRepo`, so the two never drift.
